...
def fit_distance_function(contact_df, bin_df, outprfx=None):
    ...
    for i in range(z):
        tmp[x[i]] = y[i]
    tmp = sorted(tmp.items(),key=lambda s:s[0])
    x = np.array(list(sub[0] for sub in tmp))
    y = np.array(list(sub[1] for sub in tmp))
    ...
    while spl_fail:
        try:
            #print(sorted(x[k].values))
            #print(knts)
            spl = LSQUnivariateSpline(x[k], np.log10(y[k]), t=knts, k=3, ext=3)
            spl_fail = False
        except ValueError:
